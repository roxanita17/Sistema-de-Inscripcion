...
class AreaEstudioRealizadoController extends Controller
{
    ...
    public function index()
    {
        ...
        $titulo_universitario = EstudiosRealizado::where('status', true)
            ->orderBy('titulo_universitario', 'asc')
            ->get();

        return view('admin.transacciones.area_estudio_realizado.index', compact('areaEstudioRealizado', 'area_formacion', 'titulo_universitario'));
    }
...
public function store(Request $request)
{
    ...
    $validated = $request->validate([
        ...
        'titulo_universitario_id' => 'required|exists:estudios_realizados,id',
    ]);

    // Verificar si ya existe la asignación
    $existe = AreaEstudioRealizado::where('area_formacion_id', $validated['area_formacion_id'])
        ->where('titulo_universitario_id', $validated['titulo_universitario_id'])
        ->exists();
    ...
    try {
        ...
        $areaEstudio->titulo_universitario_id = $validated['titulo_universitario_id'];
        $areaEstudio->status = true;
        $areaEstudio->save();

        info('Nueva asignación creada: Área ID ' . $validated['area_formacion_id'] . ', Título ID ' . $validated['titulo_universitario_id']);
...
public function update(Request $request, $id)
{
    ...
    try {
        ...
        $asignacion->titulo_universitario_id = $validated['titulo_universitario_id'];
        $asignacion->save();
