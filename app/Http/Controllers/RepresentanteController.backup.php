...
class RepresentanteController extends Controller
{
    ...
    public function representanteView(){
        $anioEscolarActivo = $this->verificarAnioEscolar();
        
        // Cargar los representantes con las relaciones necesarias, incluyendo banco
    ...
    public function mostrarFormulario(){
        ...
        return view("admin.representante.formulario_representante", compact('estados', 'municipios', 'parroquias_cargadas', 'bancos'));
    }

    public function mostrarFormularioModuloRepresentante(){
        $estados=Estado::query()->with("municipio")->orderBy("nombre_estado","ASC")->get();
        $municipios = Municipio::with('localidades')->orderBy('nombre_municipio','ASC')->get();
        $parroquias_cargadas = Localidad::with('municipio.estado')->orderBy('nombre_localidad','ASC')->get();
        $bancos = Banco::where('status', true)->orderBy('nombre_banco', 'ASC')->get();
    ...
    public function mostrarFormularioEditar($id){
        // Cargar todas las relaciones necesarias, incluyendo municipio y parroquia si existen
        ...
        // Asegurarse de que la ocupación esté disponible en el objeto representante
        // para que esté accesible en el frontend
        ...
        return view("modules.representante.formulario_representante", compact(
            'representante', 'estados', 'municipios', 'parroquias_cargadas', 'bancos'
        ));
    }
    ...
    public function guardar(Request $request)
    {
        ...
        $validatedData = $request->validate([
            'tipo-ci' => 'required|string|max:1',
            'cedula' => 'required|string|max:10|unique:personas,numero_cedula_persona',
            'primer-nombre' => 'required|string|max:50',
            'segundo-nombre' => 'nullable|string|max:50',
            'primer-apellido' => 'required|string|max:50',
            'segundo-apellido' => 'nullable|string|max:50',
            'sexo' => 'required|in:Masculino,Femenino',
            'fechaNacimiento' => 'required|date',
            'lugar-nacimiento' => 'required|string|max:255',
            'idEstado' => 'required|exists:estados,id',
            'idMunicipio' => 'required|exists:municipios,id',
            'idparroquia' => 'required|exists:localidads,id',
            'prefijo' => 'required|string|max:4',
            'telefono' => 'required|string|max:11',
            'correo' => 'nullable|email|max:100',
            'parentesco' => 'required|string|max:50',
            'tipo-cuenta' => 'required|string|max:20',
            'banco-representante' => 'required|exists:bancos,id',
    ...
    // Registro de depuración de campos geográficos
...
//Buscar representante por cédula
    ...
    public function consultar(Request $request):JsonResponse{
        ...
        return ApiResponse::success($representante,"Estudiante consultado",200);
    }
     ...
     */
    public function verificarCedulaExistente(Request $request)
    {
        $cedula = $request->input('cedula');
        $personaExistente = Persona::where('numero_cedula_persona', $cedula)->first();
    ...
    public function guardar(Request $request)
    {
        ...
        try {
            // Crear la persona
            $persona = new Persona();
            $persona->tipo_cedula_persona = $request->input('tipo-ci');
            $persona->numero_cedula_persona = $request->input('cedula');
            $persona->nombre_uno = $request->input('primer-nombre');
            $persona->nombre_dos = $request->input('segundo-nombre');
            $persona->apellido_uno = $request->input('primer-apellido');
            $persona->apellido_dos = $request->input('segundo-apellido');
            $persona->sexo = $request->input('sexo');
            $persona->fecha_nacimiento_personas = $request->input('fechaNacimiento');
            $persona->lugar_nacimiento_persona = $request->input('lugar-nacimiento');
            $persona->prefijo_telefono_personas = $request->input('prefijo');
            $persona->telefono_personas = $request->input('telefono');
            $persona->estado_id = $request->input('idEstado');
            $persona->municipio_id = $request->input('idMunicipio');
            $persona->localidad_id = $request->input('idparroquia');
            $persona->save();

            // Crear el representante
            $representante = new Representante();
            $representante->persona_id = $persona->id;
            $representante->correo_representante = $request->input('correo');
            $representante->parentesco = $request->input('parentesco');
            $representante->tipo_cuenta_representante = $request->input('tipo-cuenta');
            $representante->banco_id = $request->input('banco-representante');
            $representante->direccion_representante = $request->input('direccion-habitacion');
            
            // Campos opcionales
            if ($request->has('pertenece-organizacion') && $request->input('pertenece-organizacion') === 'si') {
                $representante->pertenece_a_organizacion_representante = true;
                $representante->cual_organizacion_representante = $request->input('cual-organizacion');
            } else {
                $representante->pertenece_a_organizacion_representante = false;
            }
            ...
            DB::rollBack();
            \Log::error('Error al guardar representante: ' . $e->getMessage());
            \Log::error($e->getTraceAsString());
            
            return response()->json([
                'success' => false,
                'message' => 'Error al guardar el representante: ' . $e->getMessage()
            ], 500);
        }
    }
}
