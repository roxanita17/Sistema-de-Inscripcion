/**
 * Validaciones en tiempo real para el módulo de Discapacidades
 */

console.log('Cargando validaciones para discapacidad.js');
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function limpiarFormulario() {
        const form = document.getElementById('formCrearDiscapacidad');
        if (form) {
            // Limpiar errores
...
function inicializarValidacionesCrear() {
    ...
    if (!form) {
        console.error('No se encontró el formulario con ID formCrearDiscapacidad');
        return;
    }

    // Elementos del formulario
    const nombreInput = document.getElementById('nombre_discapacidad');
    const contenedorAlerta = document.getElementById('contenedorAlertaCrear');
    ...
    // Función para validar el formato del nombre de la discapacidad
    function validarFormatoNombre(valor) {
        return valor.trim().length > 0 && valor.trim().length <= 255;
    }

    // Función para verificar si la discapacidad ya existe
    async function verificarDiscapacidadExistente(nombre) {
        try {
            const url = new URL('/admin/discapacidad/verificar', window.location.origin);
            ...
            console.error('Error al verificar la discapacidad:', error);
            return false; // En caso de error, asumimos que no existe para no bloquear al usuario
        }
    }
    ...
    if (nombreInput) {
            ...
            if (valor) {
                if (!validarFormatoNombre(valor)) {
                    ...
                    timeoutId = setTimeout(async () => {
                        ...
                        const existe = await verificarDiscapacidadExistente(valor);
                        console.log('Resultado de verificación:', existe);
                        if (existe) {
                            mostrarError(this, 'Ya existe una discapacidad con este nombre');
                        } else if (haHechoBlur) {
                            mostrarError(this, '');
                        }
                    }, 500);
        ...
        nombreInput.addEventListener('blur', function() {
            ...
            if (!valor) {
                mostrarError(this, 'El nombre de la discapacidad es obligatorio');
            } else if (!validarFormatoNombre(valor)) {
                mostrarError(this, 'El nombre no puede tener más de 255 caracteres');
            } else {
                // Verificar duplicados al salir del campo
                verificarDiscapacidadExistente(valor).then(existe => {
    ...
    form.addEventListener('submit', async function(event) {
        ...
        if (nombreInput) {
            ...
            if (!valor) {
                mostrarError(nombreInput, 'El nombre de la discapacidad es obligatorio');
                esValido = false;
            } else if (!validarFormatoNombre(valor)) {
                ...
                if (existe) {
                    mostrarError(nombreInput, 'Ya existe una discapacidad con este nombre');
                    esValido = false;
                }
            }
        }
