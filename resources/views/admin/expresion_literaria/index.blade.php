...
<div class="container mt-4">
    ...
    <button type="button" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#modalCrearExpresionLiteraria">
        <i class="fas fa-plus"></i> Crear Expresión literaria
    </button>
        ...
        <table class="table table-striped align-middle text-center" id="tablaAnioEscolar">
            ...
            <tbody id="tbodyAnioEscolar">
                @if ($expresionLiteraria->isEmpty())
                                <tr>
                                    <td colspan="4" style="text-align: center;">No se encontraron expresiones literarias.</td>
                                </tr>
                            @endif
                
                @foreach ($expresionLiteraria as $datos)
                @if ($datos->status == true)
                    <tr>
                        <td>{{ $datos->letra_expresion_literaria }}</td>
                        <td>
                            @if ($datos->status == true)
                                ...
                                <div class="modal-dialog" role="document">
                                    <div class="modal-content">
                                        <div class="modal-body">
                                            ¿Estás seguro de que deseas eliminar esta expresión literaria?
                                        </div>
                                        <div class="modal-footer">
                                            <form action="{{ url('admin/expresion_literaria/' . $datos->id) }}" method="POST">
                                                @csrf
                                                @method('DELETE')
