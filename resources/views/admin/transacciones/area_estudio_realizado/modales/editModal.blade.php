    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content modal-modern">
            ...
            <div class="modal-body-edit">
                <form action="{{ route('admin.transacciones.area_estudio_realizado.modales.update', $datos->id) }}"
                    method="POST">
                    @csrf
                    <div class="form-group-modern">
                        ...
                        <select name="area_formacion_id" id="area_formacion_id_{{ $datos->id }}"
                            class="form-control-modern selectpicker-edit-{{ $datos->id }}" data-live-search="true"
                            required>
                        ...
                        <select name="estudios_id" id="estudios_id_{{ $datos->id }}"
                            ...
                            @foreach ($estudios as $titulo)
                                <option value="{{ $titulo->id }}"
                                    {{ old('estudios_id', $datos->estudios_id) == $titulo->id ? 'selected' : '' }}>
                                    {{ $titulo->estudios }}
                                </option>
                            @endforeach
                        </select>
