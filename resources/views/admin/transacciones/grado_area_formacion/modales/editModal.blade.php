                ...
                <form action="{{ route('admin.transacciones.grado_area_formacion.modales.update', $datos->id) }}" method="POST">
                    ...
                    <div class="form-group-modern">
                        ...
                        <select name="area_formacion_id" id="area_formacion_id_{{ $datos->id }}" 
                                class="form-control-modern selectpicker" 
                                data-live-search="true" title="Seleccione un área de formación" required>
...
$(document).ready(function () {
    // Inicializa los selectpicker sin duplicar opciones
    $('#viewModalEditar{{ $datos->id }}').on('shown.bs.modal', function () {
        $('#grado_id_{{ $datos->id }}, #area_formacion_id_{{ $datos->id }}').each(function() {
