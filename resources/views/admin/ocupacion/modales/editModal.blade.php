    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content modal-modern">
            ...
            <div class="modal-body-edit">
                <form action="{{ route('admin.ocupacion.modales.update', $datos->id) }}" method="POST">
                    @csrf
                    <input type="hidden" name="id" value="{{ $datos->id }}">
                    <div class="form-group-modern">
                        <label for="nombre_ocupacion_{{ $datos->id }}" class="form-label-modern">
                            Nombre de la Ocupacion
                        </label>
                        <input type="text" 
                               class="form-control-modern" 
                               id="nombre_ocupacion_{{ $datos->id }}" 
                               name="nombre_ocupacion" 
                               type="text"
                               inputmode="text"
                               maxlength="100"
                               placeholder="Edite el nombre de la ocupacion"
                               value="{{ $datos->nombre_ocupacion }}" 
                               required>
