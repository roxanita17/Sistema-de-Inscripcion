...
@include('admin.discapacidad.modales.createModal')
    ...
    {{-- Botón para abrir la modal de crear discapacidad --}}
    <button type="button" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#modalCrearDiscapacidad">
        <i class="fas fa-plus"></i> Crear Discapacidad
    </button>

    {{-- Tabla de discapacidades --}}
    <div class="table-responsive">
        <table class="table table-striped align-middle text-center" id="tablaDiscapacidad">
            ...
            <tbody id="tbodyDiscapacidad">
                @if ($discapacidad->isEmpty())
                                <tr>
                                    <td colspan="4" style="text-align: center;">No se encontraron discapacidades.</td>
                                </tr>
                            @endif
                @foreach ($discapacidad as $datos)
                @if ($datos->status == true)
                    <tr>
                        <td>{{ $datos->nombre_discapacidad }}</td>
                        <td>
                            @if ($datos->status == true)
                                ...
                                <div class="modal-dialog" role="document">
                                    <div class="modal-content">
                                        ...
                                        <div class="modal-body">
                                            ¿Estás seguro de que deseas eliminar esta discapacidad?
                                        </div>
                                        <div class="modal-footer">
                                            <form action="{{ url('admin/discapacidad/' . $datos->id) }}" method="POST">
                                                @csrf
                                                @method('DELETE')
